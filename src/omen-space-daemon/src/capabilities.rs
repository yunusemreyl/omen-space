...
impl LinuxCapabilityClassifier {
    ...
    pub fn is_wmaa_abort_prone_board(board_id: &str) -> bool {
		...
		// 88F8: Frequent WMBX/WMBA AE_AML_BUFFER_LIMIT errors when polling WMI hwmon/profile (Issue #247)
		matches!(board_id.trim().to_uppercase().as_str(), "8BCD" | "8C75" | "878A" | "8A42" | "88F8")
	}
...
fn get_all_models() -> &'static [ModelCapabilities] {
    ...
    MODELS.get_or_init(|| vec![
        ...
        model!("88F8", "HP Victus 16-d0xxx", 2021, "Victus", { supports_fan_control_wmi: true, supports_fan_control_ec: false, supports_fan_curves: true, supports_independent_fan_curves: false, supports_rpm_readback: false, fan_zone_count: 2, has_mux_switch: false, supports_gpu_power_boost: false, supports_undervolt: false, has_four_zone_rgb: false, has_keyboard_backlight: true, allow_decoupled_wmi_thermal_policy_fallback: true, notes: "GitHub #247 — Victus 16-d0xxx (88F8). Frequent WMBX/WMBA AE_AML_BUFFER_LIMIT errors leading to WMI unresponsiveness (Max fan mode switch fails after first use). Treated as abort-prone; RPM readback disabled to prevent WMI poll spam triggering the buffer limit. Thermal profile control routes via hp_wmi/acpi fallback.".to_string() }),
    ])
}
